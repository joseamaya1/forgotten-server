...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local diamondItem = Tile(altar.position):getItemById(2145)
	if diamondItem and player:getStorageValue(641314) < 1 then
		diamondItem:remove(1)
		player:setStorageValue(641314, 1)
		altar.position:sendMagicEffect(CONST_ME_TELEPORT)
		Game.createItem(altar.itemId, 1, altar.position)
		item:transform(1946)
		addEvent(revertLever, 4 * 1000, toPosition)
	...
	local diamondItem = Tile(altar.position):getItemById(2145)
	if diamondItem and player:getStorageValue(641315) < 1 then
		diamondItem:remove(1)
		player:setStorageValue(641315, 1)
		altar.position:sendMagicEffect(CONST_ME_TELEPORT)
		Game.createItem(altar.itemId, 1, altar.position)
		item:transform(1946)
		addEvent(revertLever, 4 * 1000, toPosition)
	...
	local diamondItem = Tile(altar.position):getItemById(2145)
	if diamondItem and player:getStorageValue(641316) < 1 then
		diamondItem:remove(1)
		player:setStorageValue(641316, 1)
		altar.position:sendMagicEffect(CONST_ME_TELEPORT)
		Game.createItem(altar.itemId, 1, altar.position)
		item:transform(1946)
		addEvent(revertLever, 4 * 1000, toPosition)
