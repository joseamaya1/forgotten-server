			...
			if player:getStorageValue(Storage.BigfootBurden.QuestLine) < 30 then
				...
				if player:getStorageValue(Storage.BigfootBurden.Warzone1Access) < 1 then
					if player:removeItem(18430, 1) then
						...
						npcHandler:say("I can't let you enter the warzone teleporter one for free, unless you hand me a Deathstrike's snippet. But can still always use a red teleport crystal.", cid)
					end
				else
				...
				if player:getStorageValue(Storage.BigfootBurden.Warzone3Access) < 1 then
					if player:getStorageValue(Storage.BigfootBurden.WarzoneStatus) >= 3 then 
						if player:removeItem(18496, 1) then
							...
							npcHandler:say("I can't let you enter the warzone teleporter three for free, unless you hand me an Abyssador's lash. But can still always use a red teleport crystal.", cid)
						end
					else
				...
				if player:getStorageValue(Storage.BigfootBurden.Warzone2Access) < 1 then
					if player:getStorageValue(Storage.BigfootBurden.WarzoneStatus) >= 2 then 
						if player:removeItem(18495, 1) then
							...
							npcHandler:say("I can't let you enter the warzone teleporter two for free, unless you hand me a Gnomevil's hat. But can still always use a red teleport crystal.", cid)
						end
					else
